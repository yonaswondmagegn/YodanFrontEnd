import React, { useEffect } from 'react'
import { useDispatch,useSelector } from 'react-redux'
import { apiCallBegin } from '../../../reduxstates/Auth/authActions'
import { acceptOrder } from '../../../reduxstates/Admin/AdminReducer'
import '../adminCss/eachOrder.css'

const AcceptOrder = ({cart}) => {
  const dispatch = useDispatch()
  const adminOrders = useSelector(state=>state.admin)

  console.log()
  
  const AcceptBtnClickHandler = ()=>{
    dispatch(apiCallBegin({
      url:`store/cart/${cart?.id}/`,
      method:"put",
      data:{
        id:cart?.id,
        condition: "O",
        ordercondition: "O",
        customer: cart?.customer?.id,
        products: cart?.products,
      },
      onSuccess:res=>{
        let orderdata = res.data
        orderdata.customer = cart.customer
        dispatch(acceptOrder(orderdata))
      },
      onError:"onError"
    }))
  }
  return (
    <button onClick={AcceptBtnClickHandler} className='admin__eachorder__btn admin__eachorder__accept'>Accept</button>
  )
}

export default AcceptOrder